...
불과 얼마전까지만 위와 같이 생각했는데, 이제는 잘 모르겠다. 분명히 굉장히 적용해보고 싶은 트랜디한 아키텍처라고 생각하지만, 우리의 현실과 거리가 멀다고 생각한다. 

우리 시스템은 사실 도메인 영역이 굉장히 작고, 오히려 연동 영역이 굉장히 큰 비중을 차지한다. 그래서 굳이 도메인 영역을 완벽하게 고립시키기 위해 핵사고날 아키텍처를 적용하는 것은 오히려 시스템을 복잡하게 만드는 악수가 될 것이라는 생각이 들었다.  

차라리 우리에겐 Layered Architecture가 가장 적합하지 않을까? 라는 생각이 든다. 

Layered Architecture로 가져가되, "우리 시스템에서 가장 변경이 많은 인터페이스 부분을 완벽하게 고립시키면 충분하지 않을까?" 라는 생각이 들었다.

하지만 사실 Layered든, Hexagonal이든 우리의 거대한 시스템을 하나로 합치기 위한 솔루션은 아니라고 생각한다. 

Layered Architecture, Hexagonal Architecture는 컴포넌트 내부의 클래스간의 변경 영향도를 낮추기 위한 방식이지 내가 최종적으로 묙표로 하고 있는 Gateway를 하나의 코드베이스로 합치는 것과는 거리가 멀다고 생각이 들었다.

그런 상황에서 나를 들뜨게 만든 아키텍처는, 바로  **Clean Architecture & Monorepo** 였다.
